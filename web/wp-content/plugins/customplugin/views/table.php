...
class Entry_Table extends WP_List_Table {
    ...
    function column_title($item){
        ...
        //Build row actions
        $actions = array(
            'edit'      => sprintf('<a href="?page=%s&action=%s&entry=%s">Edit</a>',$_REQUEST['page'],'edit',$item['ID']),
    ...
    function column_cb($item){
        $value = get_object_vars($item); 
        return sprintf(
            '<input type="checkbox" name="bulk-item-selection[]" value="%2$s" />',
            /*$1%s*/ $this->_args['singular'],  //Let's simply repurpose the table's singular label ("movie")
            /*$2%s*/ $value['id']                //The value of the checkbox should be the record's id
        );
    }
    ...
    function prepare_items() {
        ...
        function usort_reorder($a,$b){
            $_a = get_object_vars($a); 
            $_b = get_object_vars($b);
            $orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'name'; //If no sort, default to title
            $order = (!empty($_REQUEST['order'])) ? $_REQUEST['order'] : 'asc'; //If no order, default to asc
            $result = strcmp($_a[$orderby], $_b[$orderby]); //Determine sort order
